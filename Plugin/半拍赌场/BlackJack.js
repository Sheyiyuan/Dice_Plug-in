  ...
  switch (val) {
    ...
    default: {
      if (val === `join` || val === '加入') {
        ...
        let wealth = Number(seal.vars.intGet(ctx, `$m余额`)[0])
        ...
        if (!addok) {
          seal.replyToSender(ctx, msg, `${name}老师已加入游戏，无法重复加入`);
        }
        else if (wealth === 0) {
          seal.replyToSender(ctx, msg, `${name}老师没有信用点用来押注，无法上桌`);
        }
        else if (stage !== "") {
          ...
          seal.replyToSender(ctx, msg, `${name}老师加入成功，待本局所有玩家加入后可输入\".下注 金额\"进行下注`)
        }
      }
      if (val === 'exit' || val === '退出') {
        let id = String(ctx.player.userId)
        ...
        seal.vars.intSet(ctx, `$m筹码`, 0)
        seal.replyToSender(ctx, msg, `${name}老师已离开游戏，桌上的筹码不会返还`);
        //在所有名单中去掉这个玩家
      }
      if (val === 'MANAGElist') {
        let patis = String(seal.vars.strGet(ctx, `$g参与名单ID`)[0])
    ...
    default: {
      if ((!isNaN(val) && val !== 0 && val2 === "") || (val === "all" && val2 === "")) {
        ...
        if (addok) {
          ...
          seal.replyToSender(ctx, msg, `${name}老师已经下过注了哦`);
        }
        else if (chip > wealth) {
          seal.replyToSender(ctx, msg, `你好像没有那么多信用点，要不少押点？`)
        }
        else if (chip > 10000) {
          ...
          wealth -= chip;
          seal.vars.intSet(ctx, `$m余额`, wealth)
          ...
          seal.replyToSender(ctx, msg, `${name}老师已下注${chip}信用点，剩余${wealth}信用点`);
          let patiparts = patis.trim().split("\n")
          let stakeparts = stakepls.trim().split("\n")
    ...
    default: {
      if (val === "") {
        ...
        if (stage !== "要牌") {
          ...
          seal.replyToSender(ctx, msg, `${name}老师已处于停牌阶段，无法继续要牌`);
        }
        else if (addok) {
          seal.replyToSender(ctx, msg, `${name}老师不在此次游戏中`);
        }
        else if (points >= 21) {
          seal.replyToSender(ctx, msg, `${name}老师的总点数为${points}，无法进行要牌`);
        }
        else if (hands === "") {
          //首次要牌，直接给两张
          ...
          seal.replyToSender(ctx, msg, `${name}老师要牌完成，现有手牌:\n${hands}\n总点数${pointsum}\n玩家可输入\".叫牌\"继续叫牌，输入\".双倍\"进行双倍下注，输入\".停牌\"进行停牌`);
        }
        else {
          let replystr = ""
          let cget = randomcard(cards)
    ...
    default: {
      if (val === "") {
        ...
        if (stage !== "要牌") {
          ...
          seal.replyToSender(ctx, msg, `${name}老师未进行首次要牌或已二次要牌，无法双倍下注`);
        }
        else if (wealth < chipon) {
          seal.replyToSender(ctx, msg, `${name}老师没有足够的信用点来双倍下注`);
        }
        else {
          let cget = randomcard(cards)
          pointsum += cget[1];
          ...
          seal.replyToSender(ctx, msg, `${name}老师已进行双倍下注并停牌，现有手牌:\n${hands}\n总点数${pointsum}\n${name}老师下注增加至${chipon}信用点，剩余${wealth}信用点\n待所有玩家停牌后，可输入\".结算\"结算本局游戏`);
        }
      }
      return seal.ext.newCmdExecuteResult(true);
    }
    ...
    default: {
      if (val === "") {
        ...
        if (stage !== "要牌") {
          ...
          seal.replyToSender(ctx, msg, `${name}老师已处于停牌阶段，无法再次停牌`);
        }
        else if (addok) {
          seal.replyToSender(ctx, msg, `${name}老师不在此次游戏中`);
        }
          ...
          stoppls += `${id}\n`
          seal.vars.strSet(ctx, `$g停牌名单ID`, stoppls)
          seal.replyToSender(ctx, msg, `${name}老师已停牌，现有手牌:\n${hands}\n总点数${pointsum}\n待所有玩家停牌后，可输入\".结算\"结算本局游戏`);
        }
      }
      return seal.ext.newCmdExecuteResult(true);
    }
  ...
  switch (val) {
    ...
    default: {
      ...
      if (addok) {
        ...
        seal.replyToSender(ctx, msg, `${name}老师还没有停牌，请停牌并等待所有玩家停牌完毕后再进行结算`);
      }
      else if (conc === 1) {
        seal.replyToSender(ctx, msg, `${name}老师已经结算过了，不能重复结算`);
      }
        ...
        replystr += `${name}老师牌：${hands}，总点数${pointsum}\n`
        conpls += `${id}\n`
        let conparts = conpls.trim().split("\n")
        ...
        if ((pointan <= 21 && pointan > pointsum) || pointsum > 21) {
          replystr += `庄家获胜，收走筹码，老师剩余${wealth}信用点`
        }
        else if (pointan === pointsum) {
          wealth += chipon
          seal.vars.intSet(ctx, `$m余额`, wealth)
          replystr += `双方平局，老师收回筹码，当前拥有${wealth}信用点`
        }
        else {
          wealth += chipon * 2
          seal.vars.intSet(ctx, `$m余额`, wealth)
          replystr += `${name}老师获胜\n`
          replystr += `${name}老师本次游戏下注为${chipon}信用点，获胜后总计拥有${wealth}信用点`
        }
        if (patiparts.length === conparts.length) {
          let cardstr = "1111111111111\n1111111111111\n1111111111111\n1111111111111\n"
          seal.vars.strSet(ctx, `$g牌组`, cardstr)
